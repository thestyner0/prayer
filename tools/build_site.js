...
const [,, dataDir, pw, outDir="."] = process.argv;
...
function findJson(dir){ // read_db out_dir: <dir>/prayer/meta.json, <dir>/prayer/meta/days/N.json  |  dbseed: meta.json, dayN.json
  const r={meta:null,days:[]};
  const walk=(d)=>fs.readdirSync(d,{withFileTypes:true}).forEach(e=>{const p=path.join(d,e.name); if(e.isDirectory()) walk(p); else if(/\.json$/.test(e.name)){ const j=JSON.parse(fs.readFileSync(p,"utf8")); if(j.meeting&&j.commonPrayer) r.meta=j; else if(j.no&&j.sections) r.days.push(j); }});
...
const D={rev:meta.rev,updated:meta.updated,updatedAt:meta.updatedAt,anchorMonday:meta.anchorMonday,anchorWeek:meta.anchorWeek,meeting:meta.meeting,commonPrayer:meta.commonPrayer,days};
...
cp.execFileSync("node",[path.join(__dirname,"encrypt.js"),path.join(__dirname,"prayer_app.html"),pw,path.resolve(outDir)],{stdio:"inherit"});
