...
const [,, src, pw, out, wrapperPath] = process.argv;
if(!src||!pw||!out){ console.error("usage: node encrypt.js <html> <password> <outdir>"); process.exit(1); }
const html=zlib.gzipSync(fs.readFileSync(src),{level:9}); // gzip → AES (브라우저에서 DecompressionStream으로 해제)
...
const enc=Buffer.concat([c.update(html), c.final(), c.getAuthTag()]);
const tpl=fs.readFileSync(wrapperPath||path.join(__dirname,"wrapper.html"),"utf8");
const page=tpl
